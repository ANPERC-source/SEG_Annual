# coding=utf8
# Aim of this program is to retrieve data from the SEG database
# and to print the graph with requested industrial companies or country academy
...
def count_countr():
# Database of countries with Alpha2,3; ISO; numeric codes 
    country_dict = {"Afghanistan" : {'Alpha-2': 'AF', 'Alpha-3': 'AFG', 'Numeric code': '004', 'Link to ISO 3166-2': 'ISO 3166-2:AF', 'Independent': 'Yes'},
    ...
    "Iran" : {'Alpha-2': 'IR', 'Alpha-3': 'IRN', 'Numeric code': '364', 'Link to ISO 3166-2': 'ISO 3166-2:IR', 'Independent': 'Yes'},
    ...
    "United Kingdom of Great Britain and Nothern Ireland" : {'Alpha-2': 'GB', 'Alpha-3': 'GBR', 'Numeric code': '826', 'Link to ISO 3166-2': 'ISO 3166-2:GB', 'Independent': 'Yes'},
...
# We read the input data from the terminal
    items = input('To quit program, please type n\nIf you want to get a list of all industry companies, type: industry list\nIf you want to get a list of all countries, type: academia list\nPlease quote the company or and country of academia you want to check.\nExample1: \'BP\'\nIf more than one please use the list format:\nExample2:  [\'Saudi Aramco\', \'Chevron Energy Technology Company\', \'Petrochina\', \'China\']\nPlease type company or country you want to check here:\n')
    if items == 'n': 
        continue
    if items == 'industry list' or items == 'academia list': 
        countr_list = count_countr()
        conn = sqlite3.connect(database_name)
        cursor = conn.cursor()
        cursor = conn.execute("SELECT * FROM "+database_name.replace('.sqlite', ''))
        data_in=cursor.fetchall()
        
        SEG_countr_list = []
        SEG_industry_list = []
        for element in data_in:
            if element[0] in countr_list and sum(ast.literal_eval(element[1]))>0:
                SEG_countr_list.append(element[0])
            elif element[0] == 'Unknown academia':
                SEG_countr_list.append(element[0])
            elif element[0] not in countr_list and sum(ast.literal_eval(element[1]))>0 and element[0]  != 'Unknown academia':
                SEG_industry_list.append(element[0])
        
        if items == 'industry list':
            for element in SEG_industry_list:
                print(element)
            print()
            continue
            
        if items == 'academia list':
            for element in SEG_countr_list:
                print(element)
            print()
            continue
    ...
    legend_to_print = []
    summ_total = 0
    # We consider the case when we have only one company/country to print
    ...
    # We consider the case when we have a number of conpany(s)/country(s) to print
    ...
    # We normalize each number of affiliations by average number of authors per paper at corresponding year
    ...
    # Here we are plotting the requestqed company(s)/country(s):
