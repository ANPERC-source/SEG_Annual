# Aim of this procedure is to retrieve data from grams.sqlite and print words and phrases
...
import ssl
import ast
# numb_pages - Number of pages for corresponding year 1982-2019, 38 years in total
...
# numb_papers - Number of articles for corresponding year 1982-2019, 38 years in total
...
def make_float(my_list):
    ...
    return outputlist
# Here we are reading input data from the terminal
...
start = time.time()
if len(items) == 0: print('Error. Imput is empty string. Please try again.\n'); quit()
...
if isinstance(data, str):
    ...
    item = data
    conn = sqlite3.connect('grams.sqlite')
    cursor = conn.cursor()
# Here we are checking if the entry exists in the database
...
# We consider the case when we have few word(s)/phrase(s) to print
...
all_words_norm = []
# Here we check if data is not zero
for vals in data_to_print:
    if isinstance(vals, list):
# Normalizing each number of occurrences by number of pages at corresponding year
...
# Setting approptiaye scale for all elements in the list
for vals in ts:
# If we have a number of enrties
    if isinstance(vals, list):
...
print('Calculation time:', round(end-start, 3))
plt.show()
